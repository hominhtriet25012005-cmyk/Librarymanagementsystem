...
export default function SubscriptionsPage() {
  ...
  async function subscribe() {
    ...
    try {
      ...
      if (!checkoutUrl) {
        throw Object.assign(new Error("Invalid checkout URL"), {
          userMessage: "Máy chủ không trả về liên kết thanh toán HTTPS hợp lệ.",
        });
      }
      setPayment({ ...result, checkoutUrl });
      setSelectedPlan(null);
      setNotice({ severity: "success", text: "Đã tạo đăng ký chờ. Hãy kiểm tra thông tin trước khi mở Razorpay." });
  ...
  return <section aria-labelledby="subscriptions-title" className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <Alert severity="info" className="mb-5">Giai đoạn 8 tạo liên kết thanh toán Razorpay. Việc xác minh tự động sau thanh toán sẽ được hoàn thiện ở giai đoạn 9.</Alert>
    {notice && <Alert severity={notice.severity} className="mb-5" onClose={() => setNotice(null)}>{notice.text}</Alert>}
    {payment && <Card className="mb-5 border border-indigo-200 p-5"><h2 className="text-lg font-bold text-slate-900">Liên kết thanh toán đã sẵn sàng</h2><p className="my-2 text-sm text-slate-600">Mã giao dịch: {payment.transactionId} · Số tiền: {formatMoney(payment.amount)}</p><Button component="a" href={payment.checkoutUrl} target="_blank" rel="noopener noreferrer" variant="contained">Tiếp tục đến Razorpay</Button></Card>}
    ...
    <Dialog open={!!selectedPlan} onClose={() => { if (!busy) setSelectedPlan(null); }} fullWidth maxWidth="xs"><DialogTitle>Xác nhận chọn gói</DialogTitle><DialogContent>{selectedPlan && <div className="pt-1"><p>Bạn muốn đăng ký <strong>{selectedPlan.name}</strong> trong {selectedPlan.durationDays} ngày?</p><p className="mt-3 text-xl font-bold">{formatMoney(selectedPlan.price, selectedPlan.currency || "INR")}</p><Alert severity="info" className="mt-4">Hệ thống sẽ tạo liên kết thanh toán Razorpay. Gói chỉ hoạt động sau khi giao dịch được xác minh.</Alert></div>}</DialogContent><DialogActions><Button disabled={busy} onClick={() => setSelectedPlan(null)}>Quay lại</Button><Button disabled={busy} variant="contained" onClick={subscribe}>{busy ? "Đang tạo..." : "Tạo liên kết thanh toán"}</Button></DialogActions></Dialog>
