...
import { Add, Autorenew, Cancel, CardMembership, Edit, HourglassBottom, Paid } from "@mui/icons-material";
import { adminMembershipApi, adminPaymentsApi, getApiErrorMessage } from "../../api";
...
const EMPTY_PLAN = { planCode: "", name: "", description: "", durationDays: "30", price: "", currency: "VND", maxBooksAllowed: "2", maxDaysPerBook: "14", displayOrder: "0", isActive: true, isFeatured: false, badgeText: "", adminNotes: "" };
...
function formFromPlan(plan) {
  return {
    ...
    durationDays: String(plan.durationDays ?? 30), price: String(plan.price ?? ""), currency: plan.currency || "VND",
...
export default function AdminMembershipPage() {
  ...
  async function confirmSubscriptionPayment(event) {
    event.preventDefault();
    if (!paymentTarget || !bankTransactionId.trim() || submitting.current) return;
    submitting.current = true; setBusy(true);
    try {
      await adminPaymentsApi.confirmSubscription(paymentTarget.id, bankTransactionId);
      setPaymentTarget(null); setBankTransactionId("");
      reload("Đã xác nhận thanh toán và kích hoạt thành viên.");
    } catch (requestError) {
      setNotice({ severity: "error", text: getApiErrorMessage(requestError, "Không thể xác nhận thanh toán thành viên.") });
    } finally { submitting.current = false; setBusy(false); }
  }
  ...
  return <section aria-labelledby="admin-membership-title" className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <Dialog open={!!paymentTarget} onClose={() => { if (!busy) setPaymentTarget(null); }} fullWidth maxWidth="sm"><DialogTitle>Xác nhận thanh toán thành viên</DialogTitle><DialogContent><form id="confirm-subscription-payment" onSubmit={confirmSubscriptionPayment} className="grid gap-4 pt-2"><Alert severity="warning">Chỉ xác nhận sau khi đã đối chiếu tài khoản MB và nhận đủ {formatMoney(paymentTarget?.price, paymentTarget?.currency || "VND")} từ {paymentTarget?.userName}.</Alert><TextField required autoFocus label="Mã giao dịch ngân hàng" value={bankTransactionId} helperText="Nhập mã giao dịch hiển thị trong lịch sử nhận tiền của MB để tránh xác nhận trùng." onChange={(event) => setBankTransactionId(event.target.value)} /></form></DialogContent><DialogActions><Button disabled={busy} onClick={() => setPaymentTarget(null)}>Quay lại</Button><Button type="submit" form="confirm-subscription-payment" color="success" variant="contained" disabled={busy || !bankTransactionId.trim()}>{busy ? "Đang xác nhận..." : "Xác nhận và kích hoạt"}</Button></DialogActions></Dialog>
  </section>;
}
