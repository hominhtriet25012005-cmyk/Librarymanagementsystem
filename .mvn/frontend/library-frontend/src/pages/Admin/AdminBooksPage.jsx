...
export default function AdminBooksPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      booksApi.search({
        ...
        genreId,
        availableOnly: availableOnly === "available" ? true : undefined,
  ...
  }, [activeOnly, availableOnly, genreId, page, revision, search, sort]);
  ...
  return <section aria-labelledby="admin-books-title" className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
      ...
      <Button variant="contained" startIcon={<Add />} onClick={openCreate} disabled={genres.length === 0}>Tạo sách mới</Button>
    </div>
    {notice && <Alert severity={notice.severity} className="mb-5">{notice.text}</Alert>}
    {genres.length === 0 && !loading && <Alert severity="warning" className="mb-5">Hãy tạo ít nhất một thể loại trong mục “Quản lý thể loại” trước khi thêm sách.</Alert>}
    <div className="mb-5 grid gap-4 sm:grid-cols-3">
      <Card className="p-5"><p className="text-sm text-slate-500">Sách phù hợp</p><p className="mt-2 text-3xl font-bold">{data.totalElements}</p></Card>
      <Card className="p-5"><p className="text-sm text-slate-500">Đang hiển thị</p><p className="mt-2 text-3xl font-bold text-indigo-600">{data.content.length}</p></Card>
      <Card className="p-5"><p className="text-sm text-slate-500">Thể loại hoạt động</p><p className="mt-2 text-3xl font-bold text-emerald-600">{genres.length}</p></Card>
    </div>
    <Card className="mb-5 p-4">
      <div className="grid gap-4 lg:grid-cols-12">
        <TextField className="lg:col-span-4" label="Tìm kiếm" placeholder="Tên sách, tác giả hoặc ISBN..." value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} slotProps={{ input: { startAdornment: <Search className="mr-2 text-slate-400" /> } }} />
        <FormControl className="lg:col-span-2"><InputLabel id="book-filter-genre-label">Thể loại</InputLabel><Select labelId="book-filter-genre-label" label="Thể loại" value={genreId} onChange={(event) => { setGenreId(event.target.value); setPage(0); }}><MenuItem value="">Tất cả</MenuItem>{genres.map((genre) => <MenuItem key={genre.id} value={genre.id}>{genre.name}</MenuItem>)}</Select></FormControl>
        <FormControl className="lg:col-span-2"><InputLabel id="book-filter-available-label">Tình trạng</InputLabel><Select labelId="book-filter-available-label" label="Tình trạng" value={availableOnly} onChange={(event) => { setAvailableOnly(event.target.value); setPage(0); }}><MenuItem value="">Tất cả</MenuItem><MenuItem value="available">Còn sách</MenuItem></Select></FormControl>
        <FormControl className="lg:col-span-2"><InputLabel id="book-filter-active-label">Hiển thị</InputLabel><Select labelId="book-filter-active-label" label="Hiển thị" value={activeOnly} onChange={(event) => { setActiveOnly(event.target.value); setPage(0); }}><MenuItem value="">Tất cả</MenuItem><MenuItem value="active">Đang hoạt động</MenuItem></Select></FormControl>
        <FormControl className="lg:col-span-2"><InputLabel id="book-sort-label">Sắp xếp</InputLabel><Select labelId="book-sort-label" label="Sắp xếp" value={sort} onChange={(event) => { setSort(event.target.value); setPage(0); }}><MenuItem value="createdAt:DESC">Mới tạo trước</MenuItem><MenuItem value="title:ASC">Tên A–Z</MenuItem><MenuItem value="author:ASC">Tác giả A–Z</MenuItem><MenuItem value="publishedDate:DESC">Xuất bản mới nhất</MenuItem></Select></FormControl>
      </div>
      <div className="mt-3 flex justify-end"><Button onClick={clearFilters}>Xóa bộ lọc</Button></div>
    </Card>
    {loading ? <div className="flex min-h-64 items-center justify-center"><CircularProgress aria-label="Đang tải danh sách sách" /></div> : error ? <Alert severity="error" action={<Button onClick={() => { setLoading(true); setRevision((value) => value + 1); }}>Thử lại</Button>}>{error}</Alert> : data.content.length === 0 ? <Alert severity="info">Chưa có sách phù hợp.</Alert> : <>
      <div className="overflow-x-auto rounded-xl border bg-white shadow-sm"><table className="min-w-full text-left text-sm"><thead className="bg-indigo-50 text-slate-700"><tr><th className="px-4 py-3">Sách</th><th className="px-4 py-3">ISBN</th><th className="px-4 py-3">Thể loại</th><th className="px-4 py-3">Bản sách</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Thao tác</th></tr></thead><tbody className="divide-y">{data.content.map((book) => <tr key={book.id} className="align-top"><td className="px-4 py-4"><div className="flex min-w-64 gap-3">{book.coverImageUrl ? <img src={book.coverImageUrl} alt={`Bìa ${book.title}`} className="h-16 w-12 rounded object-cover" /> : <div className="flex h-16 w-12 items-center justify-center rounded bg-indigo-100 text-xl font-bold text-indigo-600">{book.title?.charAt(0)}</div>}<div><p className="font-semibold text-slate-900">{book.title}</p><p className="mt-1 text-slate-500">{book.author}</p><p className="mt-1 text-xs text-slate-400">{book.publicationDate || "Chưa có ngày xuất bản"}</p></div></div></td><td className="px-4 py-4 text-slate-600">{book.isbn}</td><td className="px-4 py-4">{book.genreName || "Chưa phân loại"}</td><td className="px-4 py-4"><span className={Number(book.availableCopies) > 0 ? "font-semibold text-emerald-600" : "font-semibold text-rose-600"}>{book.availableCopies ?? 0}</span>/{book.totalCopies ?? 0}</td><td className="px-4 py-4"><Chip size="small" color={book.active === false ? "default" : "success"} label={book.active === false ? "Đã ẩn" : "Đang hoạt động"} /></td><td className="px-4 py-4"><div className="flex justify-end gap-1"><Button component={Link} to={`/books/${book.id}`} size="small">Xem</Button><Button size="small" startIcon={<Edit />} onClick={() => openEdit(book)}>Sửa</Button>{book.active !== false && <Button color="error" size="small" startIcon={<VisibilityOff />} onClick={() => setDeleting(book)}>Ẩn</Button>}</div></td></tr>)}</tbody></table></div>
    ...
    <Dialog open={editor !== undefined} onClose={() => { if (!saving) setEditor(undefined); }} fullWidth maxWidth="md"><DialogTitle>{editor?.id ? "Cập nhật sách" : "Tạo sách mới"}</DialogTitle><DialogContent><form id="book-editor-form" onSubmit={save} className="grid gap-4 pt-2 sm:grid-cols-2">
      ...
      <TextField label="Giá sách" type="number" value={form.price} onChange={(event) => change("price", event.target.value)} error={!!formErrors.price} helperText={formErrors.price} slotProps={{ htmlInput: { min: 0, step: "0.01" } }} />
